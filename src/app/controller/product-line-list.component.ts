...
@Component({
  ...
  templateUrl: '../view/product-line-list.component.html',
...
export class ProductLineListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.datatable = tblElement.DataTable({
      'ajax': {
          'url': CommonService.api_url+"/product-line-list",
          'type': "POST",
      },
      'columns': [
   ...
   //   "dom": 'rtip', // the "r" is for the "processing" message
    ...
    $(document).ready(function() {
      ...
      $('#empTable tbody').on('click', 'tr', function () {
          ...
          // }
          // else{
            localStorage.setItem("product_line_id" , data["product_line_id"]);
            window.location.href ='#/product-line-edit';
  ...
  navigateToEquipmentMappingAdd = function () {
    this.router.navigateByUrl('/product-line-add');
  };
