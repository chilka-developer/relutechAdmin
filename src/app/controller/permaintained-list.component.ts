...
@Component({
  selector: 'app-permaintained-list',
  templateUrl: '../view/permaintained-list.component.html',
  styleUrls: ['../../assets/css/common-list.component.css']
})
export class PermaintainedListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.datatable = tblElement.DataTable({
      'ajax': {
          'url': CommonService.api_url+"/permaintained-list",
          'type': "POST",
      },
      'columns': [
          { data: 'maintained_id'},
    ...
    $(document).ready(function() {
      ...
      $('#empTable tbody').on('click', 'tr', function () {
          ...
          // }
          // else{
            localStorage.setItem("maintained_id" , data["maintained_id"]);
            window.location.href ='#/permaintained-edit';
  ...
  navigateToEquipmentMappingAdd = function () {
    this.router.navigateByUrl('/permaintained-add');
  };
